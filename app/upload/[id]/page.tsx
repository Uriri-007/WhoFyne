...
import { ArrowLeft, ThumbsDown, ThumbsUp, User } from 'lucide-react';
import Link from 'next/link';
import { PageTransition } from '@/src/components/Navigation';
import { getDefaultAvatar, type UploadRow } from '@/src/lib/supabase';
...
export async function generateMetadata(
  ...
  const title = `${upload.title || 'Untitled Capture'} • curator: ${upload.profiles?.username || 'Unknown curator'} | ${appName}`;
  const description = `This masterpiece has ${upload.total_votes} total votes on ${appName}. Check it out, share the vibrance, and cast your vote!`;
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-neutral-100 dark:border-neutral-800 shadow-sm transition-colors">
                    <img
                      src={upload.profiles?.avatar_url || getDefaultAvatar(upload.uploader_id)}
                      alt={upload.profiles?.username || 'Curator'}
                  ...
                  <div>
                    ...
                    <p className="font-bold text-neutral-900 dark:text-neutral-50 transition-colors">{upload.profiles?.username || 'Unknown curator'}</p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white dark:bg-neutral-900 p-6 rounded-3xl border border-neutral-100 dark:border-neutral-800 shadow-sm flex flex-col items-center transition-colors">
                  <p className="text-3xl font-black text-indigo-600 dark:text-indigo-400 mb-1 transition-colors">{upload.total_votes}</p>
